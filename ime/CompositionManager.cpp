...
HRESULT CompositionManager::executeEdit(
    ...
    case EditAction::CommitStackShortcut: {
        ...
        // R1.14 NORMAL STACK ONLY.
        //
        // Always use candidate #1 for the raw syllable currently being typed.
        // Do not reuse a possibly stale popup selection index from an older
        // word. This is important for:
        //
        //   တက် + k + Ctrl+Enter
        //          k -> က (#1)
        //   result  -> တက္က
        //
        // The special R1.8a င်/င်္ stack path is handled BEFORE this action
        // by CommitKinziShortcut and is intentionally untouched.
        const auto candidates =
            currentCandidateTexts(1);
        ...
        if (SUCCEEDED(result)) {
            debugLog(
                "R1.14 normal C+ASAT Ctrl+Enter stack committed"
            );
        }
